...
const getTheme = function (config) {
  ...
  return desktopEnv()
    ...
    .then(function ({ environment, gtkTheme }) {
      ...
      const decorationLayout = execSync(
      ...
      const supportedButtons = arrayOfButtons
        .filter((button) => button !== "appmenu")[0]
        .split(",");

      const globalTheme = getFolder(`/usr/share/themes/${themeName}`);
      const userTheme = getFolder(`${homedir()}/.themes/${themeName}`);
      const snapTheme = getFolder(
        `/snap/${themeName.toLocaleLowerCase()}/current/share/themes/${themeName}`
      );
      ...
      const getCSS = function (path, r = 0) {
        ...
        try {
          ...
          // TODO: Make GTK version configurable
          cssString = fs.readFileSync(path.replace(/3\.0/g, "3.20"), {
            encoding: "utf8",
          });
        }
        ...
        while (ostr !== cssString && qc < 10) {
          ...
          cssString = cssString.replaceAll(
            /@import url\("resource:\/\/(.*)"\);/g,
            function (m, fil) {
              let q = execSync("gresource extract gtk.gresource " + fil, {
                encoding: "utf8",
                cwd: dirname(path),
              });
              return q;
        ...
        let overrides = [
          ...
          [
            /\-gtk\-outline\-bottom\-right\-radius\:/g,
            "border-bottom-right-radius:",
          ],
          [
            /\-gtk\-outline\-bottom\-left\-radius\:/g,
          ...
          [
            /\*::-webkit-scrollbar-corner slider/g,
          ...
          [
            /:not\(:backdrop\)([^\s]*? \+ [^\s]*?)?:not\(:backdrop\)/g,
            ":not(:backdrop)",
          ],
        ...
        if (imports) {
          for (let i = 0; i < imports.length; i++) {
            let cssPath = /(@import url\(")([\w|\-|\.|\/]+)("\))/g.exec(
              imports[i]
            )[2];
        ...
        if (r === 0) {
          cssString +=
            "* {outline: none !important; user-select: none !important;} a {cursor: pointer;}";
        }
        // orig+"\n"+
        return cssString;
      };
