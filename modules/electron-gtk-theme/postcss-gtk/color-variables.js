    ...
    return function (css, processor) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        for (let k in color_vars) {
          color_vars[k] = color_vars[k].replace(
            /@([a-z_\-0-9]+)/gi,
            function (m, name) {
              // console.log("N",name)
              if (color_vars[name]) {
      ...
      return css.walk(function (node) {
        if (node.type === "decl") {
          ...
          if (
            node.prop.includes("border") &&
            node.parent.selector.includes(".gtk-button")
          ) {
            node.important = true;
          }
          ...
          if (
            node.prop === "padding" &&
            (node.parent.selector + "" === ".combo"||node.parent.selector + "" === ".gtk-entry")
          ) {
            node.remove();
          }
