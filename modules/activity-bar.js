define([
  ...
  class _CompositeBar extends compositeBar.CompositeBar {
    constructor(items, options) {
      if (
        options &&
        (options.compositeSize == 50 || options.compositeSize == 52) &&
        (options.orientation == 1 || options.orientation == 2)
      ) {
        // action bar
  ...
  moveActivityBarToBottomLegacy1 = function (theme) {
      ...
      function (original, args) {
        ...
        if (
          !this.layoutService.isVisible(
            "workbench.parts.activitybar" /* ACTIVITYBAR_PART */
          )
        ) {
          return;
        }
    ...
    override(
      ...
      function (original) {
        ...
        const sideBorderColor =
          this.getColor("sideBar.border") || this.getColor("contrastBorder");
        const borderColor =
          this.getColor("activityBar.border") ||
        ...
        container.style.borderRightWidth =
          sideBorderColor && isPositionLeft ? "1px" : null;
        container.style.borderRightStyle =
          sideBorderColor && isPositionLeft ? "solid" : null;
    ...
    override(layout.Layout, "layoutGrid", function (original) {
      ...
      let activityBarInGrid = this.workbenchGrid.hasView(
      ...
      if (
        !titlebarInGrid &&
        windowService.getTitleBarStyle(
      ...
      if (!sidebarInGrid) {
        this.workbenchGrid.addView(
          ...
          this.state.sideBar.width !== undefined
            ? this.state.sideBar.width
            : "split" /* Split */,
          panelInGrid &&
            this.state.sideBar.position === this.state.panel.position
            ? this.panelPartView
            : this.editorPartView,
          this.state.sideBar.position === 1 /* RIGHT */
            ? 3 /* Right */
            : 2 /* Left */
        );
        sidebarInGrid = true;
      ...
      if (!panelInGrid) {
        this.workbenchGrid.addView(
          this.panelPartView,
          this.getPanelDimension(this.state.panel.position) !== undefined
            ? this.getPanelDimension(this.state.panel.position)
            : "split" /* Split */,
          this.editorPartView,
          this.state.panel.position === 2 /* BOTTOM */
            ? 1 /* Down */
            : 3 /* Right */
        );
        panelInGrid = true;
    ...
    override(
      activitybarActions.ViewletActivityAction,
      "run",
      function (original) {
  ...
  let CustomizeActivityBarLegacy1 = class CustomizeActivityBarLegacy1 {
    constructor(configurationService, themeService) {
      if (
        configurationService.getValue("customizeUI.activityBar") === "bottom"
      ) {
  ...
  moveActivityBarToBottomLegacy2 = function (theme) {
    ...
    override(
      ...
      function (original, args) {
        ...
        if (this.globalActivityActionBar) {
          availableWidth -=
            this.globalActivityActionBar.viewItems.length * actionWidth; // adjust width for global actions showing
        }
        this.compositeBar.layout(new dom.Dimension(availableWidth, height));
      }
    );
    ...
    override(layout.Layout, "createWorkbenchLayout", function (original) {
      ...
      grid.isGridBranchNode = function (node) {
        ...
        else return prevIsGridBranchNode(node);
      };
    ...
    override(layout.Layout, "setSideBarHidden", function (original, argumen) {
      let hidden = argumen[0];
  ...
  moveActivityBarToPosition = function (
    theme,
    hideSettings,
    activityBarPosition,
    moveStatusbar
    ...
    if (activitybarActions.ViewContainerActivityAction) {
      override(
        activitybarActions.ViewContainerActivityAction,
        "run",
        replacement
      );
    }
    ...
    layout.Layout.prototype._updateActivityBar = function (visible) {
      ...
      if (visible) {
        ...
        if (this.state.sideBar.position === 0 /* Left */) {
          this.workbenchGrid.moveViewTo(this.activityBarPartView, [
            order,
            1 - order,
          ]);
        } else {
  ...
  let CustomizeActivityBar = class CustomizeActivityBar {
    constructor(configurationService, telemetry, themeService) {
      let activityBarPosition = configurationService.getValue(
      ...
      switch (activityBarPosition) {
        ...
        case "bottom":
          ...
          let hideSettings = configurationService.getValue(
            "customizeUI.activityBarHideSettings"
          );
          let moveStatusbar = configurationService.getValue(
            "customizeUI.moveStatusbar"
          );
          moveActivityBarToPosition(
            theme,
            hideSettings,
            activityBarPosition,
            moveStatusbar
  ...
  let CustomizeGTK = class CustomizeGTK {
    constructor(configurationService, telemetry, themeService) {
      ...
      const applyT = () => {
        ...
        override(
          compositeBarActions.ActivityActionViewItem,
          "updateStyles",
          function (original) {
            original();
            if (this.container) {
        ...
        override(
          tabsTitleControl.TabsTitleControl,
          "doRedrawTabActive",
          function (original, args) {
            ...
            original();
            tabContainer.classList.add("gtk-tab");
            tabContainer.parentElement.classList.add("gtk-tabs");
            [...tabContainer.children].forEach((c) =>
              c.classList.add("gtk-widget")
            );

            // p.parentElement.style.height="auto";
            tabContainer.parentElement.classList.add("top");
            tabContainer.parentElement.parentElement.classList.add("top");
            tabContainer.parentElement.parentElement.classList.add(
              "gtk-header"
            );
            tabContainer.parentElement.parentElement.parentElement.classList.add(
              "gtk-notebook"
            );
            tabContainer.style = "";
        ...
        setInterval(() => {
          let v = [
            ...document.body.querySelectorAll(
              ".monaco-text-button,.action-item>.label"
            ),
          ];
          // for (let p of v){
          ...
          let oo = [
            ...document.body.querySelectorAll(
              ".monaco-inputbox,.suggest-input-container,.scm-view .scm-editor-container"
            ),
          ];
          for (let p of oo) {
            if (true) {
          ...
          for (let p of document.body.querySelectorAll(
            ".monaco-workbench .part.panel>.composite.title>.panel-switcher-container>.monaco-action-bar .action-item,.monaco-workbench .part.panel>.composite.title>.panel-switcher-container>.monaco-action-bar .gtk-tab"
          )) {
            ...
            [...p.querySelectorAll(".action-label")].forEach(
        ...
        //   }
          ...
          for (let p of [
            ...document.body.querySelectorAll(".monaco-keybinding-key"),
          ]) {
          ...
          // }
          for (let p of [
            ...document.body.querySelectorAll(".monaco-custom-checkbox"),
          ]) {
          ...
          for (let p of [
            ...document.body.querySelectorAll(
              ".setting-value-checkbox,.was-setting-value-checkbox"
            ),
          ]) {
